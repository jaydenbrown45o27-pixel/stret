...
if _G.LyznSBL then pcall(function() _G.LyznSBL.Destroy() end) task.wait(0.3) end
...
local ShootConfig = require(ReplicatedStorage:WaitForChild("Modules"):WaitForChild("ShootConfig"))

local scScript = LocalPlayer.PlayerScripts:FindFirstChild("ShootController")
local gameG = getsenv(scScript)._G

local ShootCtrl = gameG.ShootController
local MeterCtrl = gameG.MeterController

local State = {
    Running = true,
    Connections = {},
    AutoRelease = true,
    CapturedPerfectPos = 0.75,
    CapturedPerfectWidth = 0.03,
...
if MeterCtrl and MeterCtrl.SetPerfectZone then
    local origSetPerfect = MeterCtrl.SetPerfectZone
    MeterCtrl.SetPerfectZone = function(self, pos, width, ...)
        State.CapturedPerfectPos = pos
        State.CapturedPerfectWidth = width
        return origSetPerfect(self, pos, width, ...)
    end
    State._origSetPerfect = origSetPerfect
end

if ShootCtrl then
    local origFire = ShootCtrl.Fire
    ShootCtrl.Fire = function(self, ...)
        if State.AutoRelease then
            self.LastReleaseTier = "PERFECT"
            self.LastReleaseError = 0
            self.LastReleaseProgress = State.CapturedPerfectPos
        end
        return origFire(self, ...)
    end
    State._origFire = origFire

    local origStartCharge = ShootCtrl.StartCharge
    ShootCtrl.StartCharge = function(self, ...)
        local result = origStartCharge(self, ...)
        if State.AutoRelease and self.IsCharging then
            local meterDuration = ShootConfig.MeterDuration or 0.5
            local perfectPos = State.CapturedPerfectPos or 0.75
            local releaseDelay = perfectPos * meterDuration
            task.delay(releaseDelay, function()
                if self.IsCharging and not self.AlreadyFired then
                    self:Fire()
                end
            end)
        end
...
table.insert(conns, RunService.Heartbeat:Connect(function(dt)
    ...
    local extraSpeed = (State.CFrameSpeedMult - 1) * hum.WalkSpeed
    local offset = moveDir.Unit * extraSpeed * dt
    root.CFrame = root.CFrame + offset
end))

local SAVE_FILE = "lyzn_sbl_outfit.json"
local HttpService = game:GetService("HttpService")

local function saveOutfit(equipped)
    pcall(function()
        writefile(SAVE_FILE, HttpService:JSONEncode(equipped))
    end)
end
...
local function setupUnlockAll()
    local uiFolder = LocalPlayer.PlayerScripts:FindFirstChild("UI")
    if not uiFolder then return end

    local PC = require(uiFolder:WaitForChild("PlayerCosmetics"))
    local CC = require(ReplicatedStorage:WaitForChild("CharacterCatalog"))
    local ClothingStyles = require(ReplicatedStorage:WaitForChild("ClothingStyles"))
    local ShoesStyles = require(ReplicatedStorage:WaitForChild("ShoesStyles"))
    local HairStyles = require(ReplicatedStorage:WaitForChild("HairStyles"))
    local AccessoriesStyles = require(ReplicatedStorage:WaitForChild("AccessoriesStyles"))
    local ProfileAction = ReplicatedStorage:WaitForChild("ProfileEvents"):WaitForChild("ProfileAction")
    local ProfileSync = ReplicatedStorage:WaitForChild("ProfileEvents"):WaitForChild("ProfileSync")
    ...
    task.spawn(function()
        local batch = 0
        for catName, catItems in pairs(CC.Categories) do
            for _, item in ipairs(catItems) do
                if type(item) == "table" and item.id then
                    allOwned[item.id] = true
                    count = count + 1
                    batch = batch + 1
                    if batch >= 50 then
                        batch = 0
        ...
        PC.HydrateFromServer({ coins = 999999, rep = 99999, owned = allOwned })
    end)

    task.wait(0.1)

    if not State._unlockOldNc then
        local oldNc
        oldNc = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
            local method = getnamecallmethod()
            if State.UnlockAllActive and method == "FireServer" and self == ProfileAction then
                local args = {...}
                if args[1] == "equip" or args[1] == "purchase" then
                    return
                end
            end
            if method == "SetCore" and self == StarterGui then
                ...
                if args[1] == "SendNotification" and type(args[2]) == "table" then
                    local txt = args[2].Text or ""
                    local title = args[2].Title or ""
                    if txt:find("Equipped") or txt:find("Acquired") or title:find("Equipped") or title:find("Acquired") then
                        return
                    end
                end
            end
        ...
        State._unlockOldNc = oldNc
    end

    local function applyVisual(char, category, itemId)
        pcall(function()
            if category == "tops" or category == "bottoms" then
                ClothingStyles.ApplyTo(char, category, itemId)
            elseif category == "shoes" then
                ShoesStyles.ApplyTo(char, itemId)
            elseif category == "hair" then
                HairStyles.ApplyTo(char, itemId)
            elseif category == "bags" or category == "chains" or category == "accessories" then
                AccessoriesStyles.ApplyTo(char, itemId)
            end
        end)
    end

    State._snapshotCooldown = false

    if not State._syncConn then
        State._syncConn = ProfileSync.OnClientEvent:Connect(function(action, data)
            if State.UnlockAllActive and action == "snapshot" then
                State._snapshotCooldown = true
                task.defer(function()
                    task.wait()
                    PC.HydrateFromServer({ owned = allOwned, coins = 999999, rep = 99999 })
                    if State._lockedOutfit then
                        local char = LocalPlayer.Character
                        if char then
                            for cat, itemId in pairs(State._lockedOutfit) do
                                task.wait()
                                pcall(function() applyVisual(char, cat, itemId) end)
                            end
                        end
                    end
        ...
        table.insert(conns, State._syncConn)
    end

    if not State._equipConn then
        State._equipConn = PC.OnEquipChanged.Event:Connect(function(category, itemId)
            if not State.UnlockAllActive then return end
            if State._snapshotCooldown then return end
            local char = LocalPlayer.Character
            if not char then return end

            applyVisual(char, category, itemId)

            if not State._lockedOutfit then State._lockedOutfit = {} end
            State._lockedOutfit[category] = itemId
            saveOutfit(State._lockedOutfit)
        end)
        table.insert(conns, State._equipConn)
    end

    State.UnlockAllActive = true

    local saved = loadOutfit()
    if saved then
        State._lockedOutfit = saved
        task.defer(function()
            State._snapshotCooldown = true
            local char = LocalPlayer.Character
            if not char then
            ...
            for category, itemId in pairs(saved) do
                pcall(function() PC.Equip(category, itemId) end)
                task.wait(0.2)
                pcall(function() applyVisual(char, category, itemId) end)
                task.wait(0.1)
            end
            task.delay(0.5, function()
                State._snapshotCooldown = false
...
function State.Destroy()
    State.Running = false
    if ShootCtrl then
        if State._origFire then ShootCtrl.Fire = State._origFire end
        if State._origStartCharge then ShootCtrl.StartCharge = State._origStartCharge end
    end
    if MeterCtrl and State._origSetPerfect then
        MeterCtrl.SetPerfectZone = State._origSetPerfect
    end
    if State._unlockOldNc then
        hookmetamethod(game, "__namecall", State._unlockOldNc)
        State._unlockOldNc = nil
    end
    for _, c in pairs(conns) do pcall(function() c:Disconnect() end) end
    pcall(function()
        if guiParent:FindFirstChild("\0Lyzn\0") then guiParent:FindFirstChild("\0Lyzn\0"):Destroy() end
    end)
    pcall(function()
        if pg:FindFirstChild("\0Lyzn\0") then pg:FindFirstChild("\0Lyzn\0"):Destroy() end
    end)
    _G.LyznSBL = nil
end

local Window = Library:CreateWindow({ Title = "Lyzn Hub", SubTitle = "Streetball" })

local ShootingTab = Window:CreateTab("Shooting")
local shotSection = ShootingTab:CreateSection("Shot Timing", "Left")
shotSection:AddToggle({ Text = "Auto Release", Default = true, Callback = function(v) State.AutoRelease = v end })

local SpeedTab = Window:CreateTab("Speed")
local speedSection = SpeedTab:CreateSection("CFrame Speed", "Left")
speedSection:AddToggle({ Text = "CFrame Speed", Default = false, Callback = function(v) State.CFrameSpeedEnabled = v end })
speedSection:AddSlider({ Text = "Speed Multiplier", Min = 1, Max = 5, Default = 1, Decimals = 1, Callback = function(v) State.CFrameSpeedMult = v end })

local ClothingTab = Window:CreateTab("Clothing")
local unlockSection = ClothingTab:CreateSection("Unlock All", "Left")
local unlockLabel = unlockSection:AddLabel("Status: Not active")
unlockSection:AddButton({ Text = "Unlock All Clothing", Callback = function()
    local count = setupUnlockAll()
    if count then
        pcall(function() unlockLabel:Set(count .. " items unlocked! Open wardrobe to equip.") end)
    end
end })
